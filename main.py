...
class tasks:
    ...
    def sort_list_of_tasks (self,sorting_type:Literal["alphabetical","completion_date"],reverse:bool=False,extra_sorting:Literal["completion","deadline","priority"]="none"):
        def sort_key(key,varible:task):
            if extra_sorting == "completion":
                return (not varible.completion,key)
            elif extra_sorting == "deadline":
                return (not varible.completion,key)
            elif extra_sorting == "priority":
                return (lambda:4 if varible.priority!="none" else (3 if varible.priority!="low" else (2 if varible.priority!="medium" else 1)),key)

            return key
        if sorting_type == "alphabetical":
            self.list_of_tasks.sort(key=lambda x:sort_key(x.title,x))
        elif sorting_type == "completion_date":
            self.list_of_tasks.sort(key=lambda x:x.completion_date,reverse=reverse)
        elif 


zadania=tasks()
print("before sort")
for i in zadania.list_of_tasks:
    print(i)

zadania.sort_list_of_tasks("alphabetical",sort_by_completion=True,sort_by_deadline=True)
